...
The blog uses a hybrid approach for optimal SEO and simplicity:

1. **Static Generated Pages** - Individual HTML files for each post (SEO-optimized)
   - Located at `posts/YYYY/MM/post-slug.html` (organized by year/month)
   - Generated by `build_posts.py` from `post-template.html`
   - Contains baked-in meta tags for crawlers (title, description, OG tags)
   - Fetches and renders Markdown dynamically for content

2. **index.html** - Main landing page with hero section, latest posts grid (max 3), and about section
   - Uses `blog.js` for SPA-like navigation between home/blog/about sections
   - Post cards link directly to static generated pages

3. **post.html** - Legacy dynamic viewer (still works with `?post=` query param)
   - Kept for backward compatibility
   - Has fallback meta tags but limited SEO effectiveness
...
**Testing SEO tags**: View source of generated pages at `posts/YYYY/MM/post-slug.html` to verify meta tags are properly baked in.
...
- [blog.js](blog.js) - Post loading, navigation, card rendering (links to static pages)
- [post.html](post.html) - Legacy single post viewer with dynamic SEO
- [build_posts.py](build_posts.py) - Static page generator (run before deploying)
- [post-template.html](post-template.html) - Template for generated static pages
