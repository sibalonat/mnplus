...
This is where the CEO becomes the person the team relies on. Not for the architecture — for the sprint priorities and for the terminology. He's the one who decides whether a feature gets called "risk register," "obligations tracker," or something else entirely, depending on who's on the other side of the call. And it matters, because in compliance the wrong word doesn't just confuse the buyer — it lowers the perceived stakes. Compliance has fines attached to it. If your terminology makes a feature sound routine, it gets treated as routine. If it makes it sound essential, it gets treated as essential.

Imagine, as a generic example, a screen developers have been calling something flat like "evidence upload" — a mechanical CRUD form. A good technical pitcher doesn't show up on a call and describe the form. They describe the audit-ready trail the customer would hand over the day a regulator showed up. Same screen. Same database table. Different room. The team doesn't have to build anything new for that to happen — the feature just gets the name it deserves.

That's the part I find hard to do as a developer. I describe the screen. The pitcher describes the day the regulator shows up.

## What Overseeing Actually Looks Like

I've heard a lot of people say that the role of the software developer has moved from building to overseeing. Like a manager making sure things comply to a specific outcome. Less about writing code, more about making sure that before we add a new feature, the existing ones still hold up.

A concrete version of that, from a recent sprint: instead of shipping a new module, we spent the week tightening edge cases — places where a null might end up where it shouldn't, where a serializer was a little too loose, where a default would surprise you. No new feature. No demo. Just a handful of test cases, two refactors, and a quieter inbox the week after.
...
I think the reason a technical pitch lands differently is that it carries an implicit guarantee. When the pitcher renames a flat-sounding feature as the audit-ready trail, the customer hears it and on some level trusts that the person saying it knows whether the thing on the other end can actually hold up under audit. They're not selling a vision into a vacuum. They're selling something they know is one or two engineering decisions away from being true. That confidence reads, even if it never gets said.
